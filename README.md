...
| Variable scopes — `g:`/`s:`/`b:`/`w:`/`t:`/`l:`/`a:`/`v:` + `:set`/`&opt` (`'ignorecase'` wired into regex). Reading a scope dict whole (`keys(l:)`, `string(b:)`) lists every variable in it | Working — `tests/parity_cases/scopes_and_execute.vim`; `b:changedtick` is absent (the buffer model does not count its own edits) |
