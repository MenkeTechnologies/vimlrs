...
#
# For every vendored interpreter C file under csrc/, extract each function
# DEFINITION (column-0 return type + name + "(", not ending in ";") and emit a
# Rust stub carrying the real C name and a `csrc/<file>:<line>` citation. Stubs
...
FILES="csrc/eval.c csrc/ex_eval.c csrc/eval/buffer.c csrc/eval/decode.c \
csrc/eval/deprecated.c csrc/eval/encode.c csrc/eval/executor.c csrc/eval/fs.c \
csrc/eval/funcs.c csrc/eval/list.c csrc/eval/typval.c csrc/eval/userfunc.c \
csrc/eval/vars.c csrc/eval/window.c"
...
MODS="$OUT/mod.rs"
{
  ...
  echo "//! Neovim C function definition (real name + \`csrc/<file>:<line>\`"
