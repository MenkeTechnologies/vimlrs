...
//! streaming parser (`src/mpack/{mpack_core,object,conv}.c`: `mpack_parser_t`,
//! `mpack_node_t`, `mpack_parse`, `mpack_unpack_*`, the `MPACK_TOKEN_*` enum),
//! which is a separate C library with no counterpart in this standalone
//! interpreter. Those functions are left as honest stubs in
//! `stubs/decode.rs`; only [`positive_integer_to_special_typval`] (which is
//! self-contained) is ported here.
#![allow(
    dead_code,
    non_snake_case,
...
/// Port of `typval_parser_error_free()` from `csrc/eval/decode.c:1016` — free an
/// mpack parser's pending node state.
///
/// RUST-PORT NOTE: the msgpack decode path (`mpack_parse_typval`/`unpack_typval`)
/// is a stub pending the libmpack streaming-parser port, so there is no
/// `mpack_parser_t` node array to free here → no-op.
